...
type CollisionTuple<'tick, T, S> = (
    ...
    Vec<(&'tick BoundingBox, &'tick BoundingBox)>,
);

impl<'tick, T: Collidable, S: Collidable> From<CollisionTuple<'tick, T, S>> for Collision<'tick, T, S> {
    fn from((((id0, e0), (id1, e1)), hb_collisions): CollisionTuple<'tick, T, S>) -> Self {
        Self {
            ids: (id0, id1),
            objs: (e0, e1),
...
pub fn check_for_collision_pairs<
    'tick,
    T: Collidable,
    S: Collidable,
>(
...
mod cartesian_collision_test {
    ...
    fn ref_pair_eq_order_independent<E> (tp1: (&E,&E), tp2: (&E,&E)) -> bool {
    ...
    fn hb_collisions_test() {
        ...
        for element in correct_collisions.iter() {
            assert!(pairs.iter().filter(|a| ref_pair_eq_order_independent(**a, *element)).count() == 1);
        }
    }
    ...
    fn collisions_test() {
        ...
        if ref_pair_eq_order_independent(overlaps[0], match0) {
            assert!(ref_pair_eq_order_independent(overlaps[0], match0));
            assert!(ref_pair_eq_order_independent(overlaps[1], match1));
        } else {
            assert!(ref_pair_eq_order_independent(overlaps[1], match0));
            assert!(ref_pair_eq_order_independent(overlaps[0], match1));
        }
    }
}
